...
const list = async params => {
  ...
  const uploads = await Upload.find(query, select)
    .populate(populate)
    .sort(sort)
    .limit(limit)
    .skip(skip)
...
const update = async (uploadId, body) => {
  ...
  if (upload === null) {
    const error = {
      status: 404,
      message: 'El upload que intentas editar no existe.'
    }
    throw error
  }
...
const remove = async uploadId => {
  ...
  if (upload === null) {
    const error = {
      status: 404,
      message: 'El upload que intentas eliminar no existe.'
    }
    throw error
  }
